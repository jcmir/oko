            ...
            loop {
                ...
                loop {
                    ...
                    match read_result {
                        ...
                        Ok(Ok(_bytes_read)) => {
                            match parser::parse_heartbeat(&line) {
                                parser::HeartbeatResult::Ok(msg) => {
                                    log_to_file(&format!("Watchdog verified: {}", msg));
                                }
                                parser::HeartbeatResult::Eof => {
                                    log_to_file("Connection to CoreService named pipe closed (EOF).");
                                    restart_core_service();
                                    break;
                                }
                                parser::HeartbeatResult::Error(err_msg) => {
                                    log_to_file(&err_msg);
                                }
                                _ => {}
                            }
